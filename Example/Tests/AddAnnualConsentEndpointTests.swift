...
class AddAnnualConsentEndpointTests: XCTestCase {
    ...
    func testMissingCustomerRefIdAndRpguidAddAnnualConsent() {
        let requestModelWithMissingParameters = prepareData(customerRefId: "", rpguid: "")
        let faultyRequest = CreateConsentAnnualRequest(createConsentAnnualRequest: requestModelWithMissingParameters)
        EasyPay.shared.apiClient.createAnnualConsent(request: faultyRequest) { result in
            switch result {
            case .success(_):
                break
    ...
    func testSuccessAddAnnualConsentWithoutCustomerRefId() {
        //apiKey and hmacToken are required for this test
        let requestModelWithoutCustomerRefId = prepareData(rpguid: "")
        let request = CreateConsentAnnualRequest(createConsentAnnualRequest: requestModelWithoutCustomerRefId)
        EasyPay.shared.apiClient.createAnnualConsent(request: request) { result in
            switch result {
            case .success(_):
                self.successExpectation.fulfill()
    ...
    func testSuccessAddAnnualConsentWithoutRpguid() {
        //apiKey and hmacToken are required for this test
        let requestModelWithoutRpguid = prepareData(customerRefId: "")
        let request = CreateConsentAnnualRequest(createConsentAnnualRequest: requestModelWithoutRpguid)
        EasyPay.shared.apiClient.createAnnualConsent(request: request) { result in
            switch result {
            case .success(_):
                self.successExpectation.fulfill()
    ...
    public func prepareData(customerRefId: String = "12456",
                            rpguid: String = "test_rpguid") -> CreateConsentAnnualManualRequestModel {
